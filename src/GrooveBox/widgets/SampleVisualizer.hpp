  	...
  	if (layer == 1)
    {
      ...
      if(module)
      {
        ...
        if(module->show_sample_visualizer)
        {
          ...
          float sample_start = module->selected_track->sample_playback_settings[module->sample_visualizer_step].sample_start;
          float sample_end = module->selected_track->sample_playback_settings[module->sample_visualizer_step].sample_end;

          nvgBeginPath(vg);

          // nvgRect(vg, 0, 0, sample_start * width, 50);
          // nvgRect(vg, 0, 0, sample_start * width, 50);
          nvgRect(vg, sample_start * width, 0, (sample_end * width) - (sample_start * width), 50);
