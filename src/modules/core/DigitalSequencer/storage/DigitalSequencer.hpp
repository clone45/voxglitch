struct DigitalSequencer : VoxglitchModule
{
    ...
    */
    ...
    /* Helper functions Temporarily removed for testing
...
*/
    ...
    */
    ...
    json_t *dataToJson() override
    {
        ...
        for (int sequencer_number = 0; sequencer_number < NUMBER_OF_SEQUENCERS; sequencer_number++)
        {
            json_array_append_new(sequencer_lengths_json_array, json_integer(this->voltage_sequencers[sequencer_number].getMaxLength()));
        }
    ...
    void dataFromJson(json_t *json_root) override
    {
        ...
        if (lengths_json_array)
        {
            ...
            {
                this->voltage_sequencers[sequencer_number].setMaxLength(json_integer_value(length_json));
                this->gate_sequencers[sequencer_number].setMaxLength(json_integer_value(length_json));
            }
        }
    ...
    */
    ...
    {
        // Skip all processing for testing
        return;
        ...
        for (unsigned int i = 0; i < NUMBER_OF_SEQUENCERS; i++)
        {
            voltage_sequencers[i].setMaxLength(clamp((int)params[SEQUENCER_LENGTH_KNOBS + i].getValue(), 1, 32));
            gate_sequencers[i].setMaxLength(clamp((int)params[SEQUENCER_LENGTH_KNOBS + i].getValue(), 1, 32));
        }
