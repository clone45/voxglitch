...
//
// Notes:  I just started working on this.  I'm modeling it after the GateSequencerView
// I've flushed out the constructor, but I'm not sure if it's correct yet.  Otherwize,
// I have to go though function by function and update it.
...
namespace vgLib_v2
{
    struct VoltageSequencerView : SequencerView
    {
                ...
                if (*voltage_sequencer)
                {
                    ...
                    */
                    ...
                    for (unsigned int i = 0; i < max_sequencer_steps; i++)
                    {
                        ...
                        if (value > 0)
                        {
                            drawBar(vg, i, (value * height), height, bar_color);
                        }
        ...
        void editBar(Vec mouse_position)
        {
            float value = 1 - (mouse_position.y / height);
            value = clamp(value, 0.0, 1.0);

            double bar_width = (width / max_sequencer_steps) - bar_horizontal_padding;
