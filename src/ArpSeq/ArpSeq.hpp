...
struct ArpSeq : Module
{
    ...
    json_t *dataToJson() override
    {
        ...
        for (unsigned int page = 0; page < NUMBER_OF_PAGES; page++)
        {
            ...
            // similar to json_object_set_new(sequencers_json, "position_sequencer", autobreak_memory[memory_index].position_sequencer.serialize());

            json_object_set_new(page_json, "voltage_sequencer", pages[page].voltage_sequencer.serialize());
            json_object_set_new(page_json, "chance_sequencer", pages[page].chance_sequencer.serialize());
    ...
    void dataFromJson(json_t *root_json) override
    {
        ...
        for (unsigned int page = 0; page < json_array_size(pages_json) && page < NUMBER_OF_PAGES; page++)
        {
            ...
            if (page_json)
            {
                pages[page].voltage_sequencer.deserialize(json_object_get(page_json, "voltage_sequencer"));
                pages[page].chance_sequencer.deserialize(json_object_get(page_json, "chance_sequencer"));
            }
        }
